/*! qiniu-js v1.0.21 | Copyright 2015 by Qiniu */ ! function(global) {
			...
			return r.bind("Error", function(b) {
				return function(c, d) {
					...
					if(c.refresh(), !a.disable_statistics_report) {
						...
						statisticsLogger.log(0 === p ? ExtraErrors.NetworkError : p, o, i(c.settings.url), void 0, j(c.settings.url), e.getTime() - g._start_at.getTime(), g._start_at.getTime(), d.file.size * (d.file.percent / 100), "jssdk-" + c.runtime, g.size)
					}
				}
				...
				return function(c, d, e) {
							...
							f.open("POST", a.downtoken_url, !0), f.setRequestHeader("Content-type", "application/x-www-form-urlencoded"), f.onreadystatechange = function() {
								if(4 === f.readyState)
									if(200 === f.status) {
										...
										var h = {};
										plupload.extend(h, that.parseJSON(e.response), a), b && b(c, d, that.stringifyJSON(h))
					...
					if(n = n ? n : k.ctx, logger.debug("ctx: ", n), n) {
						...
						var v;
						u && 9 >= u ? (v = new moxie.xhr.XMLHttpRequest, moxie.core.utils.Env.swf_url = a.flash_swf_url) : v = that.createAjax(), v.open("POST", t, !0), v.setRequestHeader("Content-Type", "text/plain;charset=UTF-8"), v.setRequestHeader("Authorization", "UpToken " + that.token);
						var w = function() {
					...
					if(!a.disable_statistics_report) {
						var x = e.responseHeaders.match(/(X-Reqid\:\ )(\w*)/)[2];
						statisticsLogger.log(e.status, x, i(c.settings.url), void 0, j(c.settings.url), f.getTime() - d._start_at.getTime(), d._start_at.getTime(), d.size, "jssdk-" + c.runtime, d.size)
					}
				}
			}(m)), logger.debug("bind FileUploaded event"), r.bind("FilesRemoved", function(b, c) {
				var d = new Date;
				if(!a.disable_statistics_report)
					for(var e = 0; e < c.length; e++) statisticsLogger.log(ExtraErrors.Cancelled, void 0, i(b.settings.url), void 0, j(b.settings.url), d.getTime() - c[e]._start_at.getTime(), c[e]._start_at.getTime(), c[e].size, "jssdk-" + b.runtime, c[e].size)
